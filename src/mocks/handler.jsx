...
export const handlers = [
  ...
  http.post("/users/signin", async ({request}) => {
    try {
      ...
      // 유효성 검사
      if (!email || !password) {
        return HttpResponse.json({message: "이메일, 비밀번호를 입력하세요."}, { status: 400 });
      }

      // 이메일과 비밀번호가 일치하는 유저 찾기
      const userEmail = userListDB.find((user) => user.email === email);
      const userPassword = userListDB.find((user) => user.password === password);
      if (!userEmail) {
        return HttpResponse.json({ message: "이메일을 확인해주세요." }, { status: 404 });
      }
      if (!userPassword) {
        return HttpResponse.json({ message: "비밀번호가 틀렸습니다." }, { status: 401 });
      }
      ...
      return HttpResponse.json({ message: "서버에 오류가 발생했습니다. 잠시 후 다시 시도해주세요." }, { status: 500 });
    }
  }),
